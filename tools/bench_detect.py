...
Reports:
  ...
  2. MediaPipe HandLandmarker inference FPS for num_hands = 1, 2, 3 (CPU).
...
def bench(rgb, num_hands, delegate=None, label=""):
    ...
    rec = mp_vision.HandLandmarker.create_from_options(opts)
    mp_img = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
    # warmup
    for i in range(5):
        rec.detect_for_video(mp_img, i)
    t0 = time.time()
    for i in range(N):
        rec.detect_for_video(mp_img, 1000 + i)
    dt = time.time() - t0
    rec.close()
    fps = N / dt
    print(f"[hand] {label} num_hands={num_hands}: {fps:.1f} fps  ({1000*dt/N:.1f} ms/frame)")
    return fps
...
def main():
    ...
    try:
        ...
        print(f"[hand] GPU delegate unavailable: {type(e).__name__}: {e}")
